...
class BLD4VR(data.Dataset):
    ...
    def generateSampleFace(self, idx):
        ...
        main_pts = np.load(
            os.path.join(self.img_folder, 'landmarks', self.img_folder.split("/")[1] ,self.anno[idx]) , allow_pickle = True)
        pts = main_pts
     
       
        # print(os.path.join(self.img_folder, self.anno[idx].split('_')[0], self.anno[idx][:-7] +'.jpg'))
        img = load_image(
            os.path.join(self.img_folder, self.img_folder.split("/")[-1], self.anno[idx].split(".")[0] +'.jpg'))
